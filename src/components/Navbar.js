function Navbar({ choices, fn }) {
  ...
  return <div className="min-w-full h-10 flex flex-row">
    {choices.map((s, i) => <div className="m-1 bg-white font-kanit flex-1 text-center text-lg hover:bg-slate-50 hover:cursor-pointer flex justify-center items-center" key={i} onClick={() => fn(i)}>{s}</div>)}
  </div>
}
